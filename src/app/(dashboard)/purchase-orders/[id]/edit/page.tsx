...
export default function EditPOPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const lineItems = items.map(item => ({
      ...
      cgst_percent: item.cgst_percent || (item.igst_percent ? 0 : (item.gst_percent ?? 0) / 2),
      sgst_percent: item.sgst_percent || (item.igst_percent ? 0 : (item.gst_percent ?? 0) / 2),
      igst_percent: item.igst_percent || 0,
      cgst_amount: item.cgst_amount || (item.igst_amount ? 0 : item.gst_amount / 2),
      sgst_amount: item.sgst_amount || (item.igst_amount ? 0 : item.gst_amount / 2),
