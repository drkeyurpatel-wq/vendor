    ...
    <div>
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-2xl font-bold text-[#1B3A6B] font-mono">{dn.debit_note_number}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="card p-5">
          <h3 className="font-semibold text-[#1B3A6B] mb-3 pb-2 border-b border-gray-100">Debit Note Details</h3>
        ...
        <div className="card p-5">
          <h3 className="font-semibold text-[#1B3A6B] mb-3 pb-2 border-b border-gray-100">Amounts</h3>
          <div className="space-y-2.5 text-sm">
            ...
            <div className="flex justify-between pt-2 border-t border-gray-100"><span className="font-semibold text-[#1B3A6B]">Total Amount:</span><span className="font-bold text-lg text-[#1B3A6B]">{formatCurrency(dn.total_amount)}</span></div>
          </div>
        </div>
      </div>
      ...
      {dn.invoice && (
        <div className="card p-5 mb-6">
          <h3 className="font-semibold text-[#1B3A6B] mb-3 pb-2 border-b border-gray-100">Against Invoice</h3>
      ...
      {dn.notes && (
        <div className="card p-5 mb-6">
          <h3 className="font-semibold text-[#1B3A6B] mb-2">Notes</h3>
      ...
      {dnItems && dnItems.length > 0 && (
        <div className="card overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h3 className="font-semibold text-[#1B3A6B]">Debit Note Items</h3>
          </div>
              ...
              <tbody>
                {dnItems.map((item: any, idx: number) => (
                  <tr key={item.id}>
                    ...
                    <td><span className="text-sm font-medium">{item.item?.generic_name || item.description}</span><br /><span className="text-xs text-gray-400 font-mono">{item.item?.item_code}</span></td>
