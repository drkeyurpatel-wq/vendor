        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50">
              {[1, 2, 3, 4, 5].map(i => <th key={i} className="p-3"><div className="h-4 bg-gray-200 rounded w-20" /></th>)}
            </tr>
          </thead>
          <tbody>
            {[1, 2, 3, 4, 5, 6].map(i => (
              <tr key={i} className="border-t border-gray-100">
                {[1, 2, 3, 4, 5].map(j => <td key={j} className="p-3"><div className="h-4 bg-gray-100 rounded" /></td>)}
              </tr>
            ))}
          </tbody>
        </table>
